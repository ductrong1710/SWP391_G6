...
const useActiveJob = () => {
  ...
  const markArrived = useCallback(async (assignmentId, beforePhoto) => {
    ...
    if (beforePhoto) {
      ...
      console.log('FormData BeforeImage:', beforePhoto.name, beforePhoto.size, 'bytes');
    } else {
  ...
  const completeJob = useCallback(async (assignmentId, afterPhoto, actualWeight) => {
    const form = new FormData();
  ...
  return { job, loading, error, setError, startTrip, markArrived, completeJob, declineJob, reportIssue };
};
      ...
      {showDecline ? (
        <div>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            <button className="btn-decline-trip" onClick={() => onDecline(declineReason)}>
              Xác nhận từ chối
            </button>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          ...
          <button className="btn-decline-trip" onClick={() => setShowDecline(true)}>
            ✗ Decline
          </button>
        </div>
      )}
    ...
    <div className="bottom-action-bar">
      <label className="form-group" style={{ display: 'block', marginBottom: 8 }}>
        <span style={{ fontSize: 13, fontWeight: 600 }}>
          📷 Ảnh trước khi thu gom {beforePhoto && <span style={{ color: '#10b981' }}>✓ {beforePhoto.name}</span>}
        </span>
        <input
          ref={fileRef}
          type="file"
  ...
  return (
    <div className="bottom-action-bar">
      <div className="form-group" style={{ marginBottom: 8 }}>
        <label style={{ fontSize: 13, fontWeight: 600 }}>⚖️ Cân nặng thực tế (kg)</label>
        <input
          type="number"
          className="form-input"
          value={actualWeight}
      ...
      <label className="form-group" style={{ display: 'block', marginBottom: 8 }}>
        <span style={{ fontSize: 13, fontWeight: 600 }}>
          📷 Ảnh sau khi thu gom {afterPhoto && <span style={{ color: '#10b981' }}>✓ {afterPhoto.name}</span>}
        </span>
        <input
          ref={fileRef}
          type="file"
...
const ActionBar = ({ job, onStart, onArrived, onComplete, onDecline, onReportIssue }) => {
  ...
  if (status === JOB_STATUS.ASSIGNED)   return <AssignedActions onStart={onStart} onDecline={onDecline} />;
  if (status === JOB_STATUS.ON_THE_WAY) return <OnTheWayActions onArrived={onArrived} />;
  if (status === JOB_STATUS.ARRIVED)    return <ArrivedActions onComplete={onComplete} />;
...
const ActiveJob = () => {
  ...
  const {
    job, loading, error, setError,
    startTrip, markArrived, completeJob, declineJob, reportIssue
  } = useActiveJob();
      ...
      {!job ? <EmptyState /> : (
        <div className="job-card">
          ...
          <ActionBar
            ...
            onComplete={withErrorHandler((photo, weight) => completeJob(job.assignmentId, photo, weight))}
            onDecline={withErrorHandler((reason) => declineJob(job.assignmentId, reason))}
            onReportIssue={withErrorHandler((desc, photo) => reportIssue(job.assignmentId, desc, photo))}
          />
        </div>
      )}
    </div>
