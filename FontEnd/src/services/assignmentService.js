...
const looksLikeCollector = (u) => {
  const roles =
    u.roles ?? u.roleNames ?? u.role ?? u.roleName ?? u.roleIds ?? null;
  if (typeof roles === "string" && roles.toLowerCase().includes("collector"))
    return true;
  if (
    Array.isArray(roles) &&
    roles.some((r) => String(r).toLowerCase().includes("collector"))
  )
    return true;
  const possible = `${u.title ?? ""} ${u.position ?? ""} ${
