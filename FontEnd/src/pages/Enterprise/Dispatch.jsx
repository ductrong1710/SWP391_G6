...
const normalizeStatus = (status) => {
  ...
  if (["inprogress", "in_progress", "đang thực hiện"].includes(s))
    return "InProgress";
...
const Dispatch = () => {
  ...
  const fetchCollectionRequests = useCallback(async () => {
    try {
      ...
      console.log("📋 Collection Requests raw:", data);
  ...
  const handleAssignCollector = async (reportId) => {
    ...
    const chosen = collectors.find(
      (c) => String(getCollectorId(c)) === String(collectorId)
    );
    try {
      setAssigningId(reportId);
      setError("");
      await assignmentService.assignCollector({
        requestId: parseInt(requestId), // ← dùng requestId đã parse
  ...
  const updateStatusApi = async (reportId, nextStatus) => {
    ...
    const action =
      nextStatus.toLowerCase() === "accepted" ? "accept" : "reject";
    try {
      setActionLoading(true);
      await axios.put(
        `${API_BASE_URL}/api/waste-reports/${reportId}/${action}`,
        null,
        {
          headers: { Authorization: `Bearer ${token}` },
          timeout: 10000,
  ...
  const assignCollectorToRequest = async () => {
    let collectorToAssignId =
      selectedCollector?.id ?? selectedCollector?.userId ?? null;
                    ...
                    >
                      ...
                      <div className="request-details">
                        <div className="detail-item">
                          🗑️ {report.wasteType?.name || "Unknown"}
                        </div>
                        <div className="detail-item">
                        ...
                        <div className="detail-item">
                          <span
                            className={`status-tag status-${String(
                              report.status
                            ).toLowerCase()}`}
                          >
                            {report.status}
                            ...
                            <div className="assign-row">
                              ...
                              <button
                                ...
                                disabled={
                                  ...
                                  !selectedCollectorMap[report.id]
                                }
                ...
                <div className="details-grid">
                  ...
                  <div className="detail-section">
                    ...
                    <p className="detail-value">
                      {selectedReport.user?.username || "Unknown"}
                    </p>
                  </div>
                  <div className="detail-section">
                    <label>Email:</label>
                    <p className="detail-value">
                      {selectedReport.user?.email || "N/A"}
                    </p>
                  </div>
                  <div className="detail-section">
                    <label>Waste Type:</label>
                    <p className="detail-value">
                      {selectedReport.wasteType?.name || "Unknown"}
                    </p>
                  </div>
                  <div className="detail-section">
                  ...
                  <div className="detail-section">
                    <label>Status:</label>
                    <p
                      className={`detail-value status-${String(
                  ...
                  <div className="detail-section">
                    ...
                    <p className="detail-value">
                      {calculateAIScore(selectedReport)}
                    </p>
                  </div>
                    ...
                    return (
                      <>
                        <div className="detail-section full-width">
                          ...
                          <p className="detail-value">
                            Request #{cr.requestId ?? cr.requestId} — Status:{" "}
                            {cr.status}
                          </p>
                        </div>
                        {cr.assignedCollectorId && (
                ...
                {String(selectedReport.status).toLowerCase() === "pending" && (
                  <div className="details-actions">
                    <button
                      ...
                      disabled={actionLoading}
                      onClick={() =>
                        updateStatusApi(selectedReport.id, "Accepted")
                      }
                    >
                      ✓ Accept
                    </button>
                    <button
                      ...
                      disabled={actionLoading}
                      onClick={() =>
                        updateStatusApi(selectedReport.id, "Rejected")
                      }
                    >
                      ✕ Reject
                    </button>
                  ...
                  <div className="details-actions">
                    <div className="inline-assign">
                      ...
                      >
                        ...
                        {collectors.map((c) => (
                          <option
                            key={getCollectorId(c)}
                            value={getCollectorId(c)}
                          >
                            {getCollectorName(c)}
                            {getCollectorEmail(c)
                              ? ` (${getCollectorEmail(c)})`
                      ...
                      <button
                        ...
                        disabled={
                          actionLoading ||
                          !selectedCollectorMap[selectedReport.id]
                        }
          ...
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
              ...
              {loadingCollectors ? (
                ...
                <div className="empty-collectors">
                  ...
                  <p className="empty-title">
                    Chưa có collector nào trong lịch sử
                  </p>
                  <div className="manual-input-section">
                    ...
                    <input
                      id="collectorId"
                      type="text"
                      placeholder="Nhập ID của collector"
                ...
                <>
                  <div className="collectors-list">
                    {collectors.map((collector) => (
                      <div
                        key={getCollectorId(collector)}
                        className={`collector-item ${
                          selectedCollector?.id ===
                          (collector.id ?? collector.userId)
                            ? "selected"
                            : ""
                        }`}
                        onClick={() => {
                      ...
                      >
                        <div className="collector-info">
                          <div className="collector-avatar">
                            {(collector.fullName || collector.username || "C")
                              .charAt(0)
                              .toUpperCase()}
                          </div>
                  ...
                  <div className="manual-input-section">
                    <label htmlFor="collectorIdAlt">
                      Nhập Collector ID khác:
                    </label>
                    <input
                      id="collectorIdAlt"
                      type="text"
                      ...
                      onChange={(e) => {
                        ...
                        if (e.target.value) setSelectedCollector(null);
                      }}
                    />
                  </div>
                </>
            ...
            <div className="modal-actions">
              ...
              <button
                ...
                disabled={
                  (!selectedCollector && !manualCollectorId.trim()) ||
                  actionLoading
                }
              >
